...
- **CalVer advisory ranges no longer silently match nothing (false negative)** — `matchAffected` called `semver.satisfies` without `{ loose: true }`. Advisory ranges from OSV/GitHub frequently carry CalVer bounds with leading zeros (e.g. GitHub's `<= 2026.05.0`, or an OSV-built `>=0 <2026.03.28`), which strict semver refuses to parse — `satisfies` then silently returns `false` for every version, so a real vulnerability (reproduced against a currently-open nocodb CVE) is reported clean. `loose: true` is now set alongside `includePrerelease: true` so the range is evaluated rather than dropped.
- **Malware findings are no longer dropped on a critical-severity tie during deduplication** — `deduplicate` replaced an existing Finding only on a strict `>` severity comparison. When a malware Finding (`type='mal'`, critical) and a plain CVE Finding (critical) share a `canonicalId`, `getAdvisoriesForPackage`'s `canonical_id ASC, id ASC` ordering puts the CVE first, so the equally-critical malware Finding lost the tie and its `type='mal'` signal (and the `/vulnscan` skill's "remove, this is malware" remediation) was silently discarded. On a severity tie, a malware Finding now wins over a non-malware one, independent of input order.
- **A GitHub incremental upsert no longer downgrades an OSV malware advisory to a plain CVE** — `upsertAdvisory`'s `ON CONFLICT` clause overwrote `type`/`severity` unconditionally. An OSV `MAL-*` entry that carries a CVE alias is stored `type='mal'`/critical; GitHub's reviewed pass can then collide on the same `(id, package_name)` PK with a plain-CVE advisory and relabel the row `type='cve'` at GitHub's lower severity — dropping a known-malicious package below the `failOn` threshold. Malware classification is now sticky: `type='mal'` is preserved and its severity is not lowered by a `cve`-typed write; a `cve→mal` upgrade is still allowed. Mirrors the guard added to `upsertAdvisoryFromFullSync` in v0.2.20.
- **OSV full sync no longer overwrites a GitHub row's curated `title`/`url` on a shared-PK collision** — completing the `source='github'` guard family (ranges/severity in v0.2.18, `type` in v0.2.20, `canonical_id` in v0.2.22), `upsertAdvisoryFromFullSync` left `title`/`url` unguarded, so a later OSV mirror of a shared CVE replaced GitHub's curated title and `github.com/advisories/...` link with OSV's generic summary and `osv.dev` URL. These are documented output-contract fields rendered verbatim in `--format json` (and consumed by the `/vulnscan` skill), so they now carry the same `CASE WHEN advisories.source = 'github'` guard as their peers.
- **A future GitHub sync cursor (clock skew) no longer permanently erases an advisory window** — when the stored GitHub cursor is in the future (e.g. an NTP correction or VM snapshot restore), `syncIfStale` emitted a "forcing re-sync" informational warning but still passed the future timestamp to `syncGithubAdvisories`, which built an `updated>=<future>` filter matching nothing and then advanced the cursor to now — silently skipping every advisory published in the true `[last-good, now]` window (the warning is `informational`, so the exit code stays clean). `syncGithubSafe` now rejects a future cursor and falls back to a full pull (`since=undefined`), mirroring OSV's structurally-full behavior.
- **A tampered lockfile can no longer hide a vulnerable dependency via `link:true`** — `resolveEntry` treated any entry with `link: true` as a first-party workspace symlink and skipped it (excluded from findings and `depCount`). An attacker with lockfile write access could add `link: true` to a real `node_modules/<pkg>` registry entry — which keeps its `version` and `resolved` tarball — to make vulnscan silently skip a known-vulnerable package (exit `0`, no warning). Genuine npm workspace symlink entries under `node_modules` carry no `version`, so a `link: true` entry under `node_modules` that also has a concrete `version` is now treated as a normal dependency and scanned; `link: true` outside `node_modules` is unaffected. (The related workspace-glob vector is not exploitable — the glob branch is gated on `!isUnderNodeModules` and registry deps always resolve under `node_modules`.)
- **A non-string `workspaces` entry in a crafted lockfile no longer crashes the scan with the wrong exit code** — `rootWorkspaces` is typed `string[]` but read from untrusted lockfile JSON with no validation. A non-string element (e.g. `"workspaces": [123]`) reached `matchesGlob().split()` and threw an uncaught `TypeError`, which the CLI's top-level catch maps to exit `1` ("findings") — misreporting a crash as a scan result. The glob matcher now skips non-string globs.
- **Terminal escape sequences in advisory/lockfile fields can no longer spoof the text report** — `renderGrouped` interpolated `Finding.name`/`version`/`via` and `advisory.id`/`title`/`url` directly into stdout. Some are attacker-influenceable (an npm-alias key becomes `via`; advisory metadata comes from upstream feeds), so a crafted value with ANSI/OSC control sequences could clear the screen, reposition the cursor, or overlay a fake "✓ No findings" summary over real findings. These fields (and warning messages) now have C0/C1 control characters stripped before display. The `--format json` path was already safe (JSON escaping neutralizes control characters).
